...
import highwaysFakerFactory from "./highwaysFakerFactory";
...
const HighwaysFakerDialogComponent = (props) => {
    ...
    const onRun = async () => {
        let fakeData = highwaysFakerFactory(props.user,fakerCount);

        setLoading(true);
        const promises = fakeData.map((elem) => client.service("highways").create(elem));
...
export default connect(mapState, mapDispatch)(HighwaysFakerDialogComponent);
